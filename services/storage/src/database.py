...
class Database:
    ...
    async def create_indexes(self):
        ...
        collection = self.client[settings.MONGO_DB_NAME]["mailboxes"]
        await collection.create_index("mailbox_id", unique=True)

        existing_indexes = await collection.index_information()
        ttl_index = existing_indexes.get("created_at_1")

        # If the index exists with different TTL options, drop it first
        if ttl_index and ttl_index.get("expireAfterSeconds") != settings.MAILBOX_EXPIRY_SECONDS:
            await collection.drop_index("created_at_1")

        await collection.create_index(
            "created_at",
            expireAfterSeconds=settings.MAILBOX_EXPIRY_SECONDS,
        )
