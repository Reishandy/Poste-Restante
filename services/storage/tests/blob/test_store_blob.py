...
async def test_store_blob_success(client):
    """Happy path: Storing a new blob with token returns 201 and persists to Mongo."""
...
async def test_store_blob_duplicate_id_returns_400(client):
    ...
    res2 = await client.put(
        f"/blob/{blob_id}", json={"content": "updated", "token": "token-b"}
    )
...
async def test_store_blob_missing_token_returns_422(client):
    """Validation check: Missing token field in body returns 422."""
    response = await client.put(
        "/blob/missing-token", json={"content": "only-content"}
    )
